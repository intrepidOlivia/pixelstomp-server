...
exports.MakeGeocodingRequest = function (location, callback) {
    ...
    var request = https.request(options, function (response){
        ...
        response.on('error', function (err){
            console.error(new Date().toUTCString() + "> The following error was encountered when retrieving a Geocoding result: " + err.message);
        });
        var result = '';
    ...
    request.on('error', function (err){
        console.error(new Date().toUTCString() + "> The following error was encountered by geocoding.MakeGeocodingRequest: " + err.message);
    });
    request.end();
};
